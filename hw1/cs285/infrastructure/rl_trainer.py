...
class RL_Trainer(object):
    ...
    def collect_training_trajectories(self, itr, load_initial_expertdata, collect_policy, batch_size):
        ...
        expert_data_file = open(load_initial_expertdata, 'rb') # Added by Utsav
        expert_data = pickle.load(expert_data_file) # Added by Utsav
        if itr == 0:  #Added by Utsav
            return expert_data, 0, None # Added by Utsav

        paths, envsteps_this_batch = sample_trajectories(self.env, collect_policy, batch_size, self.params['ep_len']) # Added by Utsav
    ...
    def train_agent(self):
        ...
        for train_step in range(self.params['num_agent_train_steps_per_iter']):
            ...
            ob_batch, ac_batch, re_batch, next_ob_batch, terminal_batch = self.agent.sample(self.params['train_batch_size']) # Added by Utsav

            # TODO use the sampled data for training
            # HINT: use the agent's train function
            # HINT: print or plot the loss for debugging!
            self.agent.train(ob_batch, ac_batch, re_batch, next_ob_batch, terminal_batch) # Added by Utsav
    ...
    def do_relabel_with_expert(self, expert_policy, paths):
        ...
        for i in range(0, len(paths)):  # Loop added by Utsav
    ...
    def perform_logging(self, itr, paths, eval_policy, train_video_paths):
        ...
        eval_paths, eval_envsteps_this_batch = sample_trajectories(self.env, eval_policy, self.params['eval_batch_size'], self.params['ep_len'])  # Added by Utsav
